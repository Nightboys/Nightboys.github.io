    ...
    <ul class="menu-titles">
      ...
      >
        ...
        <a @click="menuClick(o, i)">
          ...
          <span x3 v-else-if="o.name && (!o.selectName || !o.single)">{{
            o.name
          }}</span>
        </a>
      </li>
    ...
    >
      <transition name="slide-down">
        <div
          class="menu-dialog"
          v-if="
            menus[currentMenuIdx] && menus[currentMenuIdx].type !== 'picker'
          "
        >
          <single
            @menuSubmit="submit"
            ...
            v-if="
              list[currentMenuIdx].type === 'single' ||
              list[currentMenuIdx].type === 'lineSingle'
            "
            :idx="currentMenuIdx"
            :clist="items"
          ></single>
  ...
  computed: {
    items: function () {
      return this.currentMenuIdx !== -1
        ? $.extend(true, [], this.list[this.currentMenuIdx].list)
        : [];
    },
  },
  watch: {
    ...
    reset(index) {
      if (this.showList[index].canReset) {
        ...
        this.$set(
          this.list,
          index,
          $.extend(true, {}, this.showList[index].list)
        );
        ...
        if (o.name) {
          ...
          if (o.type !== 'picker') {
            o.list.forEach((obj, idx) => {
              if (!o.single && typeof obj.hasSelectAll === 'undefined')
                obj.hasSelectAll = true;
              ...
              if (obj.list) {
                ...
                obj.list.forEach((item) => {
                  ...
                  if (item.selected) {
                    ...
                    if (!o.single) {
                      this.showList[index].showName[idx] =
                        typeof o.showName === 'function'
                          ? o.showName.call(null, o, index, item)
                          : o.showName;
                    } else {
                  ...
                  if (
                    (o.type === 'columnThird' || o.type === 'cityPicker') &&
                    item.list
                  ) {
                    if (typeof item.hasSelectAll === 'undefined')
                      item.hasSelectAll = true;
    ...
    submit(o, cmType) {
      ...
      if (typeof o.selectList != 'undefined') {
        if (spcList.includes(this.list[o.idx].type)) {
          ...
          this.list[o.idx].selectName = o.selectList[0]
            ? o.selectList[0].name || o.selectList[0].key || ''
            : ''; //多选组件
        }
      }
      ...
      if (
        this.dateRange.key &&
        (this.list[o.idx].list.length == 9 || this.list[o.idx].list.length == 3)
      ) {
        //修改this.list，保存结果
...
.menu-containerBP {
  font-family: 'PingFangSC-Regular', 'PingFang SC Regular', 'microsoft yahei',
    tahoma, arial, simsun, sans-serif;
  background: #f9f9f9;
  height: 0.64rem;
  position: fixed;
  width: 100%;
