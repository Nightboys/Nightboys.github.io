<template>
  ...
  >
        ...
        <ul class="second-ul">
          ...
          >
            ...
            <ul class="third-ul" v-if="item.expand">
              <li
                :class="{ 'menu-selected-item': item.selectAll }"
                v-if="cityType != 'v1'"
              >
                <label>
                  <input
    ...
    <div class="menu-btns">
      ...
      <span class="btn btn-ok" @click="onSubmit"> 确认 </span>
    </div>
  </form>
</template>
  ...
  methods: {
    ...
    calcCountryList(node) {
      ...
      if (this.areaList[0].name == '全国') {
        ...
        if (this.index) {
          ...
          this.$nextTick(() => {
            ourCountry.hasSelected =
              ourCountry.list.filter((d) => d.selected).length > 0;
            ourCountry.selectAll =
              ourCountry.list.filter((d) => d.selected).length ==
              ourCountry.list.length;
          });
        } else {
          if (node.name == '全国') {
            // 选中全国中的'全部'
    ...
    handleAreaList(list, parent) {
      ...
      list.forEach((item) => {
        ...
        if (
          this.defaultCheckCitys &&
          this.defaultCheckCitys.includes(item.value)
        ) {
          //默认选中项处理
          item.selected = true;
